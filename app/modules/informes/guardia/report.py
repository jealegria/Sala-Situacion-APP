"""
report.py — Informe de Guardia
================================
Toda la logica de consolidacion y generacion del informe HTML de Guardia
(Adultos, Pediatria y General).
La UI (app/ui/pages/informes/guardia.py) solo llama a generar_informe().

Para modificar el informe en el futuro, editar SOLO este archivo.

Rutas por defecto (se pueden sobreescribir desde la UI):
  - Guardia Adultos   : C:\\02_HPN\\01_Base\\guardia_adultos
  - Guardia Pediatria : C:\\02_HPN\\01_Base\\guardia_pediatria
  - Output            : C:\\03_Apps\\Sala-Situacion-APP\\Outputs\\informes

El archivo de calendario se busca automaticamente en:
  <parent_de_input_adultos>\\tablas_relacionales\\calendario_se.csv
"""
import os
import re
import json
...
DEFAULT_INPUT_ADULTOS   = Path(r"C:\02_HPN\01_Base\guardia_adultos")
DEFAULT_INPUT_PEDIATRIA = Path(r"C:\02_HPN\01_Base\guardia_pediatria")
DEFAULT_OUTPUT_PATH     = Path(r"C:\03_Apps\Sala-Situacion-APP\Outputs\informes")

CALENDARIO_FILENAME  = "calendario_se.csv"
HTML_OUTPUT_FILENAME = "reporte_guardias_semanal.html"

COLUMNAS_IMPRESCINDIBLES = {'fecha_de_ingreso', 'edad', 'id'}
COLUMNAS_FINALES = [
...
def cargar_tabla_fechas(path_fechas: Path, log: LogFn) -> pd.DataFrame | None:
    log(f"  Buscando calendario en: {path_fechas}")
    if not path_fechas.exists():
        log(f"  [WARN] No se encontro el archivo de calendario en: {path_fechas}")
        return None
    try:
        ...
        if not all(col in df_fechas.columns for col in required_cols):
            ...
            log(f"  [ERROR] Faltan columnas requeridas en calendario: {missing}")
            return None
        ...
        if len(df_fechas) < original_rows:
            log(f"  [INFO] Calendario: se omitieron {original_rows - len(df_fechas)} filas sin fecha o año valido.")

        if df_fechas.empty:
            log("  [ERROR] No quedaron filas validas en el calendario.")
            return None

        log(f"  [OK] Calendario cargado: {len(df_fechas):,} filas.")
        return df_fechas
    except Exception as e:
        log(f"  [ERROR] Error al cargar calendario: {e}")
        return None
...
def clasificar_por_texto(texto_diags, edad_dias):
    if pd.isna(texto_diags) or not str(texto_diags).strip():
        return None
    ...
    for grupo, patron in patrones.items():
        if re.search(patron, texto_convertido):
            if grupo == 'Bronquiolitis':
                if edad_dias is not None and edad_dias < EDAD_LIMITE_BQL:
                    return grupo
                else:
                    continue
            else:
...
def combinar_csv_en_carpeta(carpeta: Path, tipo_servicio_carpeta: str, df_fechas: pd.DataFrame | None, anio_reporte: int, log: LogFn) -> pd.DataFrame | None:
    if not carpeta.exists():
        log(f"  [WARN] No se encontro la carpeta: {carpeta}")
        return None
    archivos = [f for f in carpeta.iterdir() if f.is_file() and f.suffix.lower() == '.csv']
    if not archivos:
        log(f"  [WARN] No se encontraron archivos CSV en {carpeta}")
        return None

    log(f"\n  Procesando {len(archivos)} archivo(s) de {tipo_servicio_carpeta} en '{carpeta.name}'...")
    dfs = []

    for archivo in sorted(archivos):
        try:
            df = pd.read_csv(archivo, encoding='utf-8-sig', sep=';', on_bad_lines='warn', low_memory=False)
            columnas_faltantes = COLUMNAS_IMPRESCINDIBLES - set(df.columns)
            if columnas_faltantes:
                log(f"    [WARN] {archivo.name}: Faltan columnas imprescindibles {columnas_faltantes}. Saltando.")
                continue
            ...
            if parse_failures > 0:
                log(f"    [INFO] {archivo.name}: {parse_failures}/{len(df)} fechas ingreso no parseadas.")
            ...
            if 'Servicio' not in df.columns:
                df['Servicio'] = tipo_servicio_carpeta
            else:
                df['Servicio'].fillna(tipo_servicio_carpeta, inplace=True)
            ...
            dfs.append(df)
            log(f"    [OK] {archivo.name} ({len(df):,} filas)")
        except Exception as e:
            log(f"    [ERROR] {archivo.name}: {e}")
            continue

    if not dfs:
        log(f"  [WARN] No se pudieron procesar archivos para {tipo_servicio_carpeta}.")
        return None
    ...
    if df_fechas is not None and 'Fecha_Ingreso_dt_para_merge' in df_combinado.columns:
        ...
        if 'Semana' not in df_combinado.columns:
            df_combinado['Semana'] = pd.NA

        if len(df_combinado) < original_rows:
            log(f"  [INFO] Se descartaron {original_rows - len(df_combinado)} filas con fecha de ingreso no valida.")

        if not df_combinado.empty:
            df_combinado = pd.merge(
                df_combinado,
                df_fechas[['Fecha', 'Semana', 'Año_se']].rename(
            ...
            if 'Año_se_calendario' in df_combinado.columns:
                before_filter = len(df_combinado)
                ...
                diff = before_filter - len(df_combinado)
                if diff > 0:
                    log(f"  [INFO] Se filtraron {diff:,} filas fuera del año {anio_reporte}.")
                df_combinado.drop(columns=['Año_se_calendario'], inplace=True, errors='ignore')
            else:
                log(f"  [WARN] Columna 'Año_se_calendario' no presente despues del merge.")
    ...
    log(f"  [OK] {tipo_servicio_carpeta}: {len(df_combinado):,} registros finales.")
    return df_combinado


# ══════════════════════════════════════════════════════════════
#  PREPARACION DE DATOS PARA REPORTE HTML
# ══════════════════════════════════════════════════════════════

def preparar_datos_para_reporte(df_source: pd.DataFrame | None, nombre_vista: str, log: LogFn):
    if df_source is None or df_source.empty:
        log(f"  [INFO] Sin datos fuente para {nombre_vista}.")
        return ("[]", None, None, "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]", "[]")
    ...
    if 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        try:
            ...
            log(f"  [WARN] No se pudo determinar rango de semanas para {nombre_vista}: {e}")
            all_semanas_in_source = []
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_resp.empty:
            try:
                df_plot_dx_resp['Semana'] = pd.to_numeric(df_plot_dx_resp['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_resp = pd.DataFrame()
    ...
    # --- Diagnosticos Gastro-Intestinales ---
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_gastro.empty:
            try:
                df_plot_dx_gastro['Semana'] = pd.to_numeric(df_plot_dx_gastro['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_gastro = pd.DataFrame()
        if all_semanas_in_source:
    ...
    # --- Diagnosticos Cardio-Vasculares ---
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_cardio.empty:
            try:
                df_plot_dx_cardio['Semana'] = pd.to_numeric(df_plot_dx_cardio['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_cardio = pd.DataFrame()
        if all_semanas_in_source:
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_tec.empty:
            try:
                df_plot_dx_tec['Semana'] = pd.to_numeric(df_plot_dx_tec['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_tec = pd.DataFrame()
        if all_semanas_in_source:
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_ponzonosos.empty:
            try:
                df_plot_dx_ponzonosos['Semana'] = pd.to_numeric(df_plot_dx_ponzonosos['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_ponzonosos = pd.DataFrame()
        if all_semanas_in_source:
    ...
    # --- Diagnosticos Inmunoprevenibles ---
    ...
    if 'Dx' in df_source.columns and 'Semana' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_plot_dx_inmuno.empty:
            try:
                df_plot_dx_inmuno['Semana'] = pd.to_numeric(df_plot_dx_inmuno['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_plot_dx_inmuno = pd.DataFrame()
        if all_semanas_in_source:
    ...
    # --- Tabla Egresos ---
    json_tabla_egresos = "[]"
    tipos_egreso_esperados = ['Alta médica', 'Defunción', 'Derivación', 'Internación']
    if 'Semana' in df_source.columns and 'tipo_de_egreso' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_tabla_eg_prep.empty:
            try:
                df_tabla_eg_prep['Semana'] = pd.to_numeric(df_tabla_eg_prep['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_tabla_eg_prep = pd.DataFrame()
            if not df_tabla_eg_prep.empty and 'Semana' in df_tabla_eg_prep.columns and df_tabla_eg_prep['Semana'].notna().any():
                ...
                if not df_eg_counts.empty:
                    ...
                    for col_egreso in tipos_egreso_esperados:
                        if col_egreso not in df_tabla_eg_pivot.columns:
                            df_tabla_eg_pivot[col_egreso] = 0
    ...
    # --- Grafico Internacion / Derivacion ---
    ...
    if 'Semana' in df_source.columns and 'tipo_de_egreso' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_int_der_prep.empty:
            try:
                df_int_der_prep['Semana'] = pd.to_numeric(df_int_der_prep['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_int_der_prep = pd.DataFrame()
    ...
    json_tabla_franjas = "[]"
    franjas_esperadas_tabla = ['0-8 hs', '8-16 hs', '16-24 hs']
    if 'Semana' in df_source.columns and 'Franja Horaria Ingreso' in df_source.columns and not df_source['Semana'].isna().all():
        ...
        if not df_tabla_fh_prep.empty:
            try:
                df_tabla_fh_prep['Semana'] = pd.to_numeric(df_tabla_fh_prep['Semana'], errors='coerce').dropna().astype(int)
            except Exception:
                df_tabla_fh_prep = pd.DataFrame()
            if not df_tabla_fh_prep.empty and 'Semana' in df_tabla_fh_prep.columns and df_tabla_fh_prep['Semana'].notna().any():
                ...
                if not df_fh_counts.empty:
                    ...
                    for col_franja in franjas_esperadas_tabla:
                        if col_franja not in df_tabla_fh_pivot.columns:
                            df_tabla_fh_pivot[col_franja] = 0
    ...
    if all(col in df_source.columns for col in cols_for_sin_dx_check) and not df_source['Semana'].isna().all():
        ...
        if not df_sin_dx_prep.empty:
            ...
            if not df_sin_dx_prep.empty and 'Semana' in df_sin_dx_prep.columns and df_sin_dx_prep['Semana'].notna().any():
                ...
                df_sin_dx_prep['PacienteSinDiagnostico'] = (
                    df_sin_dx_prep['SinDx_Def'] &
                ...
                if not total_evaluable_per_semana.empty:
                    ...
                    df_porcentaje_sin_dx_calc.loc[mask_valid_total, 'PorcentajeSinDx'] = (
                        ...
                        df_porcentaje_sin_dx_calc.loc[mask_valid_total, 'TotalEvaluables'] * 100
                    ).round(1)
            ...
            if not df_triage_sem.empty and 'Semana' in df_triage_sem.columns and df_triage_sem['Semana'].notna().any():
                ...
                if not triage_counts.empty:
                    ...
                    for p_col in preferred_order:
                        if p_col in current_triage_columns:
                            final_column_order.append(p_col)
                    for c_col in sorted(current_triage_columns):
                        if c_col not in final_column_order:
                            final_column_order.append(c_col)

                    triage_pivot = triage_pivot.reindex(columns=final_column_order, fill_value=0)
                    for col in final_column_order:
                        if col != 'Semana':
                            triage_pivot[col] = triage_pivot[col].astype(int)
...
# ══════════════════════════════════════════════════════════════
#  GENERACION DE REPORTE HTML INTERACTIVO
# ══════════════════════════════════════════════════════════════

def generar_reporte_html_interactivo(
    datos_vistas_dict: dict,
    output_folder: Path,
    global_min_week: int,
    global_max_week: int,
    anio_reporte: int,
    ...
    js_color_definitions = """
        ...
        const specificDxColors = {
            'Diarreas Sanguinolentas': '#dc3545',
            'Gastro-Intestinales': '#007bff'
        };
...
<body>
                        ...
                        x: {{
                            ...
                            ticks: {{
                                callback: function(v, i, t) {{
                                    const maxT = 20; const lbl = this.getLabelForValue(v);
                                    if(weeks.length > maxT) {{ return (i % Math.ceil(weeks.length/maxT) === 0) ? lbl : null; }}
                                    return lbl;
                                }}
                            }}
                        }},
    ...
    report_path = output_folder / HTML_OUTPUT_FILENAME
    try:
        output_folder.mkdir(parents=True, exist_ok=True)
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        log(f"  [OK] Reporte HTML interactivo generado: {report_path.name}")
        return report_path
    except Exception as e:
        log(f"  [ERROR] Error al guardar HTML: {e}\n{traceback.format_exc()}")
        raise
...
def generar_informe(
    input_adultos: Path,
    input_pediatria: Path,
    output_path: Path,
    log: LogFn,
    anio: int = 2026,
    path_fechas: Path | None = None,
) -> None:
    """
    Consolida los datos de Guardia de Adultos y Guardia de Pediatria
    y genera el reporte HTML interactivo semanal.

    Args:
        input_adultos:   Carpeta con CSVs de guardia adultos.
        input_pediatria: Carpeta con CSVs de guardia pediatria.
        output_path:     Carpeta de salida del informe.
        log:             Callback para mensajes en consola.
        anio:            Año a filtrar y reportar.
        path_fechas:     Ruta opcional al archivo calendario_se.csv.
    """
    log("=" * 60)
    log(f"  INFORME DE GUARDIA — AÑO {anio}")
    log("=" * 60)
    log(f"  Guardia Adultos   : {input_adultos}")
    log(f"  Guardia Pediatria : {input_pediatria}")
    log(f"  Carpeta de Salida : {output_path}")

    # 1. Resolver ruta del calendario
    if path_fechas is None:
        cand1 = input_adultos.parent / "tablas_relacionales" / CALENDARIO_FILENAME
        cand2 = input_pediatria.parent / "tablas_relacionales" / CALENDARIO_FILENAME
        cand3 = Path(r"C:\02_HPN\01_Base\tablas_relacionales") / CALENDARIO_FILENAME
        if cand1.exists():
            path_fechas = cand1
        elif cand2.exists():
            path_fechas = cand2
    ...
    # 2. Cargar calendario
    log("\n--- Carga de Calendario Epidemiologico ---")
    df_fechas = cargar_tabla_fechas(path_fechas, log)
    if df_fechas is None:
        log(f"  [WARN] No se pudo cargar el calendario. El filtrado por año ({anio}) no se podra realizar con el calendario.")

    # 3. Procesar datos por servicio
    log("\n--- Procesamiento de Datos por Servicio ---")
    df_adultos = combinar_csv_en_carpeta(input_adultos, "Adultos", df_fechas, anio, log)
    df_pediatria = combinar_csv_en_carpeta(input_pediatria, "Pediatría", df_fechas, anio, log)

    df_guardia_adultos = df_adultos.copy() if df_adultos is not None else pd.DataFrame()
    df_guardia_pediatria = df_pediatria.copy() if df_pediatria is not None else pd.DataFrame()

    valid_dfs = []
    if not df_guardia_adultos.empty:
        valid_dfs.append(df_guardia_adultos)
    if not df_guardia_pediatria.empty:
        valid_dfs.append(df_guardia_pediatria)

    if not valid_dfs:
        log("\n[ERROR] No se encontraron datos validos ni para Adultos ni para Pediatria.")
        return

    df_final = pd.concat(valid_dfs, ignore_index=True)
    log(f"\n  [OK] Registros totales combinados: {len(df_final):,} filas.")

    # 4. Preparar datos para reporte HTML
    log("\n--- Preparacion de Datos para Reporte HTML ---")
    (gDxResp_gen, min_g, max_g, tEg_gen, gID_gen, tFr_gen, gCH_gen, tP_gen,
     ...
     tablaTriage_gen, triageColOrder_gen) = preparar_datos_para_reporte(df_final, f"Guardia General ({anio})", log)
    ...
    (gDxResp_adu, min_a, max_a, tEg_adu, gID_adu, tFr_adu, gCH_adu, tP_adu,
     ...
     tablaTriage_adu, triageColOrder_adu) = preparar_datos_para_reporte(df_guardia_adultos, f"Guardia Adultos ({anio})", log)
    ...
    (gDxResp_ped, min_p, max_p, tEg_ped, gID_ped, tFr_ped, gCH_ped, tP_ped,
     ...
     tablaTriage_ped, triageColOrder_ped) = preparar_datos_para_reporte(df_guardia_pediatria, f"Guardia Pediatria ({anio})", log)
    ...
    log(f"  Rango global de semanas: {global_min_week} a {global_max_week}")

    # 5. Generar reporte HTML interactivo
    log("\n--- Generacion de Reporte HTML ---")
    has_data_for_report = any(
        (isinstance(datos_para_html[key], str) and datos_para_html[key] != "[]" and datos_para_html[key] != "")
        for key in datos_para_html
    )

    if has_data_for_report or any(
        datos_para_html.get(f"{view}_tablaTriage", "[]") != "[]"
        for view in ["general", "adultos", "pediatria"]
    ):
        report_path = generar_reporte_html_interactivo(
            datos_para_html, output_path, global_min_week, global_max_week, anio, log
        )
        log("\n" + "=" * 60)
        ...
        log(f"\n  [WARN] No hay datos suficientes para generar el reporte HTML para el año {anio}.")
