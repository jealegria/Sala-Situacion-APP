...
guardia.py  (UI page)
======================
Pagina del informe de Guardia (Adultos y Pediatria).

Layout:
  - Campo Input 1 : carpeta guardia_adultos   + [Abrir] [Seleccionar]
  - Campo Input 2 : carpeta guardia_pediatria + [Abrir] [Seleccionar]
...
from app.modules.informes.guardia.report import (
    generar_informe,
    DEFAULT_INPUT_ADULTOS,
    DEFAULT_INPUT_PEDIATRIA,
    DEFAULT_OUTPUT_PATH,
)
...
class _GuardiaWorker(QThread):
    log_signal      = pyqtSignal(str)
    finished_signal = pyqtSignal()

    def __init__(self, input_adultos: Path, input_pediatria: Path, output_path: Path, anio: int):
        super().__init__()
        self._input_adultos   = input_adultos
        self._input_pediatria = input_pediatria
...
class GuardiaPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._worker: _GuardiaWorker | None = None
        self._build_ui()
        self._set_defaults()
    ...
    def _build_ui(self):
        ...
        # Titulo
        title = QLabel("Guardia")
        title.setObjectName("page_title")
        root.addWidget(title)
        root.addSpacing(4)

        subtitle = QLabel(
            "Genera el informe interactivo HTML de Vigilancia Epidemiológica de Guardia "
            "(Adultos y Pediatría) a partir de los registros consolidados de INTRANET."
        )
        ...
        # ── Campo Input Adultos ──────────────────────────────────
        root.addWidget(self._make_section_label("Carpeta Guardia Adultos (input)"))
        root.addSpacing(6)
        self._input_adultos_field, row_adultos = self._make_path_row("input_adultos")
        root.addLayout(row_adultos)
        root.addSpacing(14)

        # ── Campo Input Pediatria ────────────────────────────────
        root.addWidget(self._make_section_label("Carpeta Guardia Pediatría (input)"))
        root.addSpacing(6)
        self._input_pediatria_field, row_pediatria = self._make_path_row("input_pediatria")
        root.addLayout(row_pediatria)
        root.addSpacing(14)
    ...
    def _set_defaults(self):
        self._input_adultos_field.setText(str(DEFAULT_INPUT_ADULTOS))
        self._input_pediatria_field.setText(str(DEFAULT_INPUT_PEDIATRIA))
    ...
    def _run(self):
        input_adultos   = self._input_adultos_field.text().strip()
        input_pediatria = self._input_pediatria_field.text().strip()
        output_path     = self._output_field.text().strip()
        anio            = self._anio_spin.value()

        if not input_adultos and not input_pediatria:
            self._log("  [ERROR] Debe especificar al menos una carpeta de entrada (Adultos o Pediatria).")
            return
        if not output_path:
            self._log("  [ERROR] Selecciona una carpeta de salida.")
            return

        inp_adultos   = Path(input_adultos) if input_adultos else Path("")
        inp_pediatria = Path(input_pediatria) if input_pediatria else Path("")
        out           = Path(output_path)

        self._run_btn.setEnabled(False)
        self._worker = _GuardiaWorker(inp_adultos, inp_pediatria, out, anio)
