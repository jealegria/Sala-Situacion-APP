"""
sm_tocogineco.py  (UI page)
============================
Pagina del informe de Salud Mental y Tocoginecologia.

Layout:
  - Campo Input  : carpeta agendas_consolidado + [Abrir] [Seleccionar]
...
from app.modules.informes.sm_tocogineco.report import (
    generar_informe,
    DEFAULT_INPUT_PATH,
    DEFAULT_OUTPUT_PATH,
)
...
class SmTocoginecPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._worker: _SmTocoWorker | None = None
        self._build_ui()
        self._set_defaults()
    ...
    def _build_ui(self):
        ...
        # Titulo
        title = QLabel("SM y Tocogineco")
        title.setObjectName("page_title")
        root.addWidget(title)
        root.addSpacing(4)

        subtitle = QLabel(
            "Genera el informe HTML de Salud Mental y Tocoginecología "
            "a partir de los archivos de agendas consolidadas."
        )
        ...
        # ── Campo Input ──────────────────────────────────────────
        root.addWidget(self._make_section_label("Carpeta de agendas (input)"))
        root.addSpacing(6)
    ...
    def _run(self):
        ...
        if not input_path:
            self._log("  [ERROR] Selecciona una carpeta de agendas.")
            return
        if not output_path:
        ...
        if not inp.is_dir():
            self._log(f"  [ERROR] La carpeta de agendas no existe: {inp}")
            return

        self._run_btn.setEnabled(False)
        self._worker = _SmTocoWorker(inp, out, anio)
