"""
vacunatorio.py  (UI page)
==========================
Pagina del informe de Vacunatorio.

Layout:
  - Campo Input 1 : carpeta agendas_consolidado + [Abrir] [Seleccionar]
  - Campo Input 2 : carpeta fuera_agenda        + [Abrir] [Seleccionar]
  - Campo Input 3 : carpeta internacion_andes   + [Abrir] [Seleccionar]
  - Campo Output  : carpeta de salida           + [Abrir] [Seleccionar]
...
from app.modules.informes.vacunatorio.report import (
    generar_informe,
...
class _VacunatorioWorker(QThread):
...
class VacunatorioPage(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._worker: _VacunatorioWorker | None = None
        self._build_ui()
        self._set_defaults()
    ...
    def _build_ui(self):
        ...
        # Titulo
        title = QLabel("Vacunatorio")
        title.setObjectName("page_title")
        root.addWidget(title)
        root.addSpacing(4)

        subtitle = QLabel(
            "Genera el panel poblacional interactivo de Vacunatorio a partir de "
            "los registros de agendas consolidadas y fuera de agenda."
        )
        ...
        root.addSpacing(6)
        self._input_agendas_field, row_agendas = self._make_path_row()
        root.addLayout(row_agendas)
        root.addSpacing(12)

        # ── Campo Input Fuera Agenda ─────────────────────────────
        root.addWidget(self._make_section_label("Carpeta Fuera de agenda (input)"))
        root.addSpacing(6)
        self._input_fuera_field, row_fuera = self._make_path_row()
        root.addLayout(row_fuera)
        root.addSpacing(12)

        # ── Campo Input Internacion ──────────────────────────────
        root.addWidget(self._make_section_label("Carpeta Internación Andes (input)"))
        root.addSpacing(6)
        self._input_internacion_field, row_internacion = self._make_path_row()
        root.addLayout(row_internacion)
        root.addSpacing(12)

        # ── Campo Output ─────────────────────────────────────────
        root.addWidget(self._make_section_label("Carpeta de salida (output)"))
        root.addSpacing(6)
        self._output_field, output_row = self._make_path_row()
    ...
    def _run(self):
        input_agendas     = self._input_agendas_field.text().strip()
        input_fuera       = self._input_fuera_field.text().strip()
        input_internacion = self._input_internacion_field.text().strip()
        output_path       = self._output_field.text().strip()
        anio              = self._anio_spin.value()

        if not input_agendas and not input_fuera:
            self._log("  [ERROR] Debe especificar al menos una carpeta de entrada (Agendas o Fuera de agenda).")
            return
        if not output_path:
            self._log("  [ERROR] Selecciona una carpeta de salida.")
            return

        inp_ag    = Path(input_agendas) if input_agendas else Path("")
        inp_fu    = Path(input_fuera) if input_fuera else Path("")
        inp_inter = Path(input_internacion) if input_internacion else None
        out       = Path(output_path)

        self._run_btn.setEnabled(False)
        self._worker = _VacunatorioWorker(inp_ag, inp_fu, inp_inter, out, anio)
